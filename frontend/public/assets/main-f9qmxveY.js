(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const r of document.querySelectorAll('link[rel="modulepreload"]'))s(r);new MutationObserver(r=>{for(const a of r)if(a.type==="childList")for(const n of a.addedNodes)n.tagName==="LINK"&&n.rel==="modulepreload"&&s(n)}).observe(document,{childList:!0,subtree:!0});function e(r){const a={};return r.integrity&&(a.integrity=r.integrity),r.referrerPolicy&&(a.referrerPolicy=r.referrerPolicy),r.crossOrigin==="use-credentials"?a.credentials="include":r.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function s(r){if(r.ep)return;r.ep=!0;const a=e(r);fetch(r.href,a)}})();class l{constructor(t){this.parent=t}getHTML(t){const e=t.image&&t.image.trim()!==""?t.image:"https://via.placeholder.com/300x200?text=МедФарм";return`
        ...
        `}addListeners(t,e,s){const r=document.querySelector(`button.btn-detail[data-id="${t.id}"]`);r&&r.addEventListener("click",()=>e(t.id));const a=document.querySelector(`button.btn-delete-product[data-id="${t.id}"]`);a&&(a.addEventListener("click",n=>{n.stopPropagation(),s(t.id)}),a.addEventListener("mouseenter",()=>{a.style.background="#c82333",a.style.transform="scale(1.1)"}),a.addEventListener("mouseleave",()=>{a.style.background="#dc3545",a.style.transform="scale(1)"}))}render(t,e,s){const r=this.getHTML(t);this.parent.insertAdjacentHTML("beforeend",r),this.addListeners(t,e,s)}}class u{async _request(t,e={}){try{const s=await fetch(t,e);return s.status===204?{data:null,status:s.status}:{data:await s.json(),status:s.status}}catch(s){throw console.error("Fetch error:",s),s}}async get(t){return this._request(t,{method:"GET",headers:{"Content-Type":"application/json"}})}async post(t,e){return this._request(t,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(e)})}async patch(t,e){return this._request(t,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify(e)})}async delete(t){return this._request(t,{method:"DELETE",headers:{"Content-Type":"application/json"}})}}const c=new u;class m{constructor(){this.baseUrl=""}getProducts(){return"/api/products"}getProductsByName(t){return`/api/products?name=${encodeURIComponent(t)}`}getProductsByPrice(t){return`/api/products?price=${encodeURIComponent(t)}`}getProductsByForm(t){return`/api/products?form=${encodeURIComponent(t)}`}getProductsByDescription(t){return`/api/products?description=${encodeURIComponent(t)}`}getProductById(t){return`/api/products/${t}`}createProduct(){return"/api/products"}updateProduct(t){return`/api/products/${t}`}deleteProduct(t){return`/api/products/${t}`}}const i=new m;class p{constructor(t,e){this.parent=t,this.productId=e}get pageRoot(){return document.getElementById("product-page")}getHTML(){return`
            <div class="container">
        ...
        `}async loadProduct(){try{const{data:t,status:e}=await c.get(i.getProductById(this.productId));e===200&&t?this.renderProduct(t):e===404?this.showNotFound():this.showError("Не удалось загрузить информацию о товаре")}catch(t){console.error("Ошибка загрузки:",t),this.showError("Ошибка соединения с сервером")}}renderProduct(t){const e=document.getElementById("product-detail");if(!e)return;const s=t.image&&t.image.trim()!==""?t.image:"https://via.placeholder.com/500x400?text=МедФарм";e.innerHTML=`
            <div class="row g-0">
                <div class="col-md-6">
                    <img src="${s}"
        ...
        `;const r=document.getElementById("back-button");r&&r.addEventListener("click",()=>{new d(this.parent).render()})}showNotFound(){const t=document.getElementById("product-detail");if(t){t.innerHTML=`
            ...
            `;const e=document.getElementById("back-button-error");e&&e.addEventListener("click",()=>{new d(this.parent).render()})}}showError(t){const e=document.getElementById("product-detail");if(e){e.innerHTML=`
            ...
            `;const s=document.getElementById("retry-btn");s&&s.addEventListener("click",()=>this.loadProduct());const r=document.getElementById("back-button-error");r&&r.addEventListener("click",()=>{new d(this.parent).render()})}}render(){this.parent.innerHTML="",this.parent.insertAdjacentHTML("beforeend",this.getHTML()),this.loadProduct()}}class d{constructor(t){this.parent=t,this.products=[]}get pageRoot(){return document.getElementById("products-grid")}getHTML(){return`
            <div class="container">
        ...
        `;try{const{data:t,status:e}=await c.get(i.getProducts());e===200&&t?(this.products=t,this.renderProducts(t)):this.showError("Не удалось загрузить товары")}catch(t){console.error("Ошибка загрузки:",t),this.showError("Ошибка соединения с сервером")}}renderProducts(t){if(this.pageRoot.innerHTML="",!t||t.length===0){this.pageRoot.innerHTML=`
                <div class="alert alert-info text-center py-5">
                    <i class="fas fa-box-open fa-3x mb-3"></i>
                    <h5>Нет доступных товаров</h5>
                    <p>Нажмите кнопку "Добавить препарат", чтобы создать первый товар</p>
                </div>
            `;return}t.forEach(e=>{const s={id:e.id,name:e.name,shortDescription:e.description&&e.description.length>100?e.description.substring(0,100)+"...":e.description||"Современный эффективный препарат",image:e.image,price:e.price+" ₽"};new l(this.pageRoot).render(s,this.handleProductClick.bind(this),this.handleDeleteProduct.bind(this))})}async addProduct(t){if(!t.name||!t.price)return alert("Пожалуйста, заполните обязательные поля: Название и Цена"),{success:!1,error:"Не заполнены обязательные поля"};t.image||(t.image="https://via.placeholder.com/300x200?text=МедФарм");try{const{data:e,status:s}=await c.post(i.createProduct(),t);return s===201&&e?(await this.loadProducts(),{success:!0,product:e}):{success:!1,error:"Не удалось создать товар"}}catch(e){return console.error("Ошибка создания:",e),{success:!1,error:e.message}}}async deleteProduct(t){if(!confirm("Вы уверены, что хотите удалить этот препарат?"))return!1;try{const{status:s}=await c.delete(i.deleteProduct(t));return s===204?(await this.loadProducts(),!0):(alert("Не удалось удалить товар"),!1)}catch(s){return console.error("Ошибка удаления:",s),alert("Ошибка при удалении товара"),!1}}openAddModal(){document.getElementById("add-product-form").reset(),new bootstrap.Modal(document.getElementById("addProductModal")).show()}setupAddButton(){const t=document.getElementById("add-product-btn");t&&t.addEventListener("click",()=>this.openAddModal());const e=document.getElementById("submit-product-btn");e&&e.addEventListener("click",async()=>{const s={name:document.getElementById("product-name").value,price:document.getElementById("product-price").value,image:document.getElementById("product-image").value,description:document.getElementById("product-description").value,form:document.getElementById("product-form").value,dosage:document.getElementById("product-dosage").value,contraindications:document.getElementById("product-contraindications").value};e.disabled=!0,e.innerHTML='<i class="fas fa-spinner fa-spin me-2"></i>Добавление...';const r=await this.addProduct(s);r.success?(bootstrap.Modal.getInstance(document.getElementById("addProductModal")).hide(),this.showNotification("Товар успешно добавлен!","success")):alert("Ошибка: "+(r.error||"Не удалось добавить товар")),e.disabled=!1,e.innerHTML='<i class="fas fa-save me-2"></i>Добавить препарат'})}setupSearch(){const t=document.getElementById("search-btn");t&&t.addEventListener("click",()=>this.searchProducts());const e=document.getElementById("clear-search-btn");e&&e.addEventListener("click",()=>this.clearSearch());const s=document.getElementById("search-input");s&&s.addEventListener("keypress",r=>{r.key==="Enter"&&this.searchProducts()})}clearSearch(){document.getElementById("search-input").value="",document.getElementById("search-field").value="name",this.loadProducts()}async searchProducts(){const t=document.getElementById("search-input").value,e=document.getElementById("search-field").value;if(console.log("Поиск:",t,"Поле:",e),!t.trim()){this.loadProducts();return}this.pageRoot.innerHTML=`
            <div class="text-center py-5">
                <div class="spinner-border text-primary" role="status"></div>
                <p class="mt-3">Поиск...</p>
            </div>
        `;try{let s;switch(e){case"name":s=i.getProductsByName(t);break;case"price":s=i.getProductsByPrice(t);break;case"form":s=i.getProductsByForm(t);break;case"description":s=i.getProductsByDescription(t);break;default:s=i.getProductsByName(t)}console.log("URL запроса:",s);const a=await(await fetch(s)).json();if(this.renderProducts(a),a.length===0){this.pageRoot.innerHTML=`
                    <div class="alert alert-info text-center py-5">
                        <i class="fas fa-search fa-3x mb-3"></i>
                        <h5>Ничего не найдено</h5>
                        <p>По запросу "${t}" в поле "${e}" ничего не найдено</p>
                        <button class="btn btn-primary mt-2" id="clear-search-btn">
                            <i class="fas fa-times me-2"></i>Сбросить поиск
                        </button>
                    </div>
                `;const n=document.getElementById("clear-search-btn");n&&n.addEventListener("click",()=>this.clearSearch())}}catch(s){console.error("Ошибка поиска:",s),this.showError("Ошибка при поиске")}}showNotification(t,e="info"){const s=document.createElement("div");s.className=`alert alert-${e} alert-dismissible fade show position-fixed top-0 end-0 m-3`,s.style.zIndex="9999",s.style.minWidth="300px",s.innerHTML=`
